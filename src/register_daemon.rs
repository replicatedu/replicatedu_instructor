...
use class_crypto::serialization::{Participant,Message};
...
pub fn main(){
    ...
    let instructor_cryto_file_string = fs::read_to_string("instructor_keys.toml".to_owned())
        .expect("error reading the instructor_keys.toml file, start daemon in an initialized repo \
                folder or move the instructor_keys.toml to your location");

    let instructor_cryto_obj:Participant = toml::from_str(&instructor_cryto_file_string)
        .expect("error parsing coord crypto");

    //this is used to decrypt the students url 
    let coord_cryto_file_string = fs::read_to_string("coord_keys.toml".to_owned())
        .expect("error reading the coord_keys.toml file, start daemon in an initialized repo \
                folder or move the coord_keys.toml to your location");

    let coord_cryto_obj:Participant = toml::from_str(&coord_cryto_file_string)
        .expect("error parsing coord crypto");
    
    
    let coord_cryto = ClassCrypto::new_from_sk("coord", coord_cryto_obj.sk, true).expect("error creating cryto obj");
    let instructor_cryto = ClassCrypto::new_from_sk("instructor", instructor_cryto_obj.sk, true).expect("error creating cryto obj");
    ...
    loop {
        let did_panic = panic::catch_unwind(||{
            ...
            for reg in &open_regs{
                ...
                let student_message:Message = toml::from_str(&reg.body).expect("error reading toml");
                let plain_message = coord_cryto.decrypt_from_toml(&reg.body).expect("error decrypting");
                println!("{}", Green.paint("registration decrypted using coord keys added:"));
                println!("username: {} repo: {}",reg.title,str::from_utf8(&plain_message).expect("err parsing utf8"));        
                
                
                //encrypt the confirm message using the instructor crypto
                let confirmation = instructor_cryto.encrypt("confirm".as_bytes().to_vec(), student_message.pk);
        ...
        if did_panic.is_err(){
            println!("{}", Red.paint("panic: api url incorrect, make sure running this from student dir "));
        }
